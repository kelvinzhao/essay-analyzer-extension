...
class ArticleExtractor {
   ...
   */
  extractAuthor() {
    // 微信公众号 - 只取第一个 .rich_media_meta_text 的第一个直接文本节点
    const wechatAuthor = document.querySelector('.rich_media_meta_text');
    if (wechatAuthor) {
      // 获取第一个非空文本节点
      let text = '';
      for (const node of wechatAuthor.childNodes) {
      ...
      // 如果没有找到文本节点，回退到使用 textContent 但清理空白
      if (!text) {
        text = wechatAuthor.textContent.trim().replace(/\s+/g, ' ');
      }
      return text;
    }
